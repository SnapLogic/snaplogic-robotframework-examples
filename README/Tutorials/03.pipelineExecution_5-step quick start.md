...
- **[Windows WSL VS Code Setup Guide](../How%20To%20Guides/infra_setup_guides/windows/windows_wsl_vscode_setup.md)** - Essential setup for WSL, Ubuntu, make commands, and VS Code integration on Windows
...
# Run Oracle tests — idempotent project-space setup happens automatically:
#   • If the project space and project don't exist, they are created.
#   • If they already exist, they are reused (nothing is deleted).
# Snaplex registration + Groundplex launch + your Oracle tests then run.
make robot-run-all-tests TAGS="oracle"

# Want to skip Snaplex re-registration / .slpropz re-download on subsequent
# runs? Use verify-only mode (fast-fail if project space is missing):
...
# Run tests using an existing (externally managed) Groundplex.
# Idempotent project-space setup happens automatically (creates if missing,
# reuses if already there). This target NEVER creates, starts, or stops a
# Groundplex — make sure yours is already running.
make robot-run-tests-no-gp TAGS="oracle"

# Verify-only / fast-fail mode (skips Snaplex registration):
make robot-run-tests-no-gp TAGS="oracle" PROJECT_SPACE_SETUP=False
```
...
- **RobotFrameWork Tests** You'll see Robot Framework output showing each step. Look for green "PASS" messages.
  ...
  **In SnapLogic Org:** (Based on the values given in .env file)
   ...
   - ProjectSpace is created **only if it does not already exist** — existing project spaces are **reused**, never deleted. Other projects in the same space are never touched. (Destructive recreate is opt-in via `FORCE_RECREATE_PROJECT_SPACE=True` with confirmation.)
   - Project is created **only if it does not already exist** in the project space; existing projects are reused as-is.
   - Snaplex definition is registered (idempotent; HTTP 409 = name collision in the org will fail the test with a clear message).
