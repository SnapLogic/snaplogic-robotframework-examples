...
## What is an environment file? (universal definition)

An **environment file** (commonly called a `.env` file) is just a plain text file that stores **configuration values as `KEY=VALUE` pairs** — things like usernames, passwords, hostnames, API URLs, ports, and feature flags. Every major framework and language uses them the same way: Python, Node.js, Java/Spring, Go, Ruby, Robot Framework, Docker Compose — all of them read `.env` files and inject the values as environment variables that the application can pick up at runtime.

The reason every framework adopts this pattern is simple: it **separates configuration from code**. Your source code stays the same across local, dev, stage, and production — only the `.env` file changes. It also keeps secrets (passwords, tokens, keys) **out of your git repository**, so credentials never get committed by accident. When you see `${ORG_NAME}` or `os.getenv("DB_PASSWORD")` anywhere in code, you're looking at a value that was loaded from a `.env` file at startup.

In short: **a `.env` file is the "settings panel" of your application** — change it without touching the code, and the application behaves differently.
...
### 2. Environment-Specific Runs (using `ENV=`)

Pass `ENV=<file>` to override values from a per-environment file at the project root. The file is loaded **last** and wins over anything else.
...
#### Production:
```bash
make robot-run-tests TAGS="oracle" ENV=.env.prod
```

#### UAT:
```bash
make robot-run-tests TAGS="oracle" ENV=.env.uat
```
...
Environment files being loaded (in order):
